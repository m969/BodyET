...
namespace ETModel
{
    public class BPhysicsWorld : Entity
    {
        ...
        public static BPhysicsWorld Get()
        {
            if (singleton == null && !_isDisposed)
            {
                ...
                //else if (ws.Length == 0)
                //{
                //    Log.Error("Need to add a dynamics world to the scene");
                //}
                //else
                //{
                //    Log.Error("Found more than one dynamics world.");
                //    singleton = ws[0];
                ...
                //    }
                //}
                singleton = EntityFactory.CreateWithParent<BPhysicsWorld>(Game.Scene);
        ...
        SolverType m_solverType = SolverType.Default;
        public SolverType solverType
        {
            get { return m_solverType; }
            set
            {
                if (value != m_solverType && m_world != null)
                {
                    Log.Error("Can't modify a Physics World after simulation has started");
                    return;
                }
        ...
        public HelperType helperType
        {
            ...
            set
            {
                if (value != m_helperType && m_world != null)
                {
                    Log.Error("Can't modify a helper type after simulation has started");
                    return;
                }
                m_helperType = value;
            }
        ...
        float m_fixedTimeStep = -1;
        public float fixedTimeStep
        {
            get
            {
                return m_fixedTimeStep != -1 ? m_fixedTimeStep : 0.03f;//Time.fixedDeltaTime;
            }
            set
            {
        ...
        */
        ...
        public int frameCount
        {
            get
            {
                ...
                {
                    return PhysicsWorldHelper.m__frameCount;
                }
                else
                {
                    return -1;
        ...
        {
            if (PhysicsWorldHelper != null) PhysicsWorldHelper.RegisterCollisionCallbackListener(toBeAdded);
        }

        public void DeregisterCollisionCallbackListener(BCollisionObject.BICollisionCallbackEventHandler toBeRemoved)
        {
            if (PhysicsWorldHelper != null) PhysicsWorldHelper.DeregisterCollisionCallbackListener(toBeRemoved);
        }
        ...
        public void RemoveRigidBody(BulletSharp.RigidBody rb)
        {
            if (!_isDisposed)
            {
                ...
                //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removing rigidbody {0} from world", rb.UserObject);
        ...
        //        {
        //            Log.Error("World type must be multibody collision");
        //            return false;
        //        }
        //        //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat(mb, "Adding multibody {0} to world", mb.name);
        //        if (mb._BuildCollisionObject())
        //        {
        //            ((MultiBodyDynamicsWorld)m_world).AddMultiBody(mb.MultiBody);
        //            mb.isInWorld = true;
        //        }
        //        return true;
        //    }
        ...
        //public void RemoveMultiBody(MultiBody mb)
        //{
        //    if (!_isDisposed)
        //    {
        //        if (m_worldType != WorldType.MultiBodyWorld)
        ...
        //        {
        //            if (mb.UserObject is BMultiBody bMultiBody)
        //                Log.LogFormat(bMultiBody, "Removing multibody {0} from world", bMultiBody.name);
        //            else
        //                Log.LogFormat("Removing multibody {0} from world", mb);
        //        }
        //        ((MultiBodyDynamicsWorld)m_world).RemoveMultiBody(mb);
        ...
        //            return false;
        //        }
        //        //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat(c, "Adding constraint {0} to world", c.name);
        ...
        public void RemoveConstraint(BulletSharp.TypedConstraint c)
        {
            if (!_isDisposed)
            {
                ...
                //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removing constraint {0} from world", c.Userobject);
        ...
        //            return false;
        //        }
        //        // //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat(c, "Adding multibody constraint {0} to world", c);
        //        //if (c._BuildConstraint())
        //        {
        ...
        //    {
        //        //if (debugType <= BDebug.DebugType.Trace) Log.LogErrorFormat("The Physics World must be a BSoftBodyWorld for adding soft bodies");
        //        return false;
        //    }
        //    if (!_isDisposed)
        //    {
        //        //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat(softBody, "Adding softbody {0} to world", softBody.name);
        ...
        //    {
        //        //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removing softbody {0} from world", softBody.UserObject);
        ...
        protected virtual void _InitializePhysicsWorld()
        {
            try
            {
                ...
                Log.Debug($"{m_world} {_ddWorld}");
                //Add a BPhysicsWorldLateHelper component to call FixedUpdate
                PhysicsWorldHelper = new BThreadedWorldHelper();
                PhysicsWorldHelper.PhysicsWorld = this;
                (PhysicsWorldHelper as BThreadedWorldHelper).Start();
        ...
        */
                                        ...
                                        out SoftBodyWorldInfo softBodyWorldInfo)
        {
            ...
            {
                Log.Error("For World Type = SoftBodyAndRigidBody collisionType must be collisionType=SoftBodyRigidBodyCollisionConf. Switching");
        ...
        {
            //if (debugType >= BDebug.DebugType.Debug) Log.Log("BDynamicsWorld Disposing physics.");
            ...
            if (m_world != null)
            {
                ...
                if (_ddWorld != null)
                {
                    //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removing Constraints {0}", _ddWorld.NumConstraints);
                    for (i = _ddWorld.NumConstraints - 1; i >= 0; i--)
                    {
                        ...
                        //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removed Constaint {0}", constraint.Userobject);
                        constraint.Dispose();
                    }
                }

                //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removing Collision Objects {0}", m_world.NumCollisionObjects);
                ...
                for (i = m_world.NumCollisionObjects - 1; i >= 0; i--)
                {
                    ...
                    {
                        //Log.Debug(body.NumConstraintRefs == 0, "Rigid body still had constraints");
                        body.MotionState.Dispose();
                    }
                    m_world.RemoveCollisionObject(obj);
                    if (obj.UserObject is BCollisionObject) ((BCollisionObject)obj.UserObject).isInWorld = false;
                    //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removed CollisionObject {0}", obj.UserObject);
                    obj.Dispose();
                }
                ...
                if (_mbdWorld != null)
                {
                    //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removing MultiBody Constraints {0}", _mbdWorld.NumMultiBodyConstraints);
                    ...
                    //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removing Multibodies {0}", _mbdWorld.NumMultibodies);
                    //remove the rigidbodies from the dynamics world and delete them
                    for (i = _mbdWorld.NumMultibodies - 1; i >= 0; i--)
                    {
                        ...
                        //if (mb.UserObject is BMultiBody bmb)
                        //    bmb.isInWorld = false;
                        //if (debugType >= BDebug.DebugType.Debug) Log.LogFormat("Removed CollisionObject {0}", mb.UserObject);
                        mb.Dispose();
                    }
                }
    ...
    public class BDefaultCollisionHandler
    {
        ...
        public void OnPhysicsStep(CollisionWorld world)
        {
            ...
            //Log.Debug($"collisionCallbackListeners Count={collisionCallbackListeners.Count}");
            //Log.Debug($"numManifolds Count={numManifolds}");
            for (int i = 0; i < numManifolds; i++)
            {
                ...
                {
                    foreach (BCollisionObject.BICollisionCallbackEventHandler collisionCallbackHandler in ((BCollisionObject)a.UserObject).collisionCallbackEventHandlers)
                ...
                {
                    foreach (BCollisionObject.BICollisionCallbackEventHandler collisionCallbackHandler in ((BCollisionObject)b.UserObject).collisionCallbackEventHandlers)
                        collisionCallbackHandler.OnVisitPersistentManifold(contactManifold);
                }
            }

...
namespace ETModel
{
    public class BGameObjectMotionState : MotionState, IDisposable
    {
        ...
        public BGameObjectMotionState(TransformComponent transform)
        {
            ...
            lock (lck)
            {
                if (mustUpdateTransform)
                {
                    ...
                    if (threadHelper != null && previousBulletTransform.TimeStamp >= 0)
                    {
                        ...
                        transform.position = UnityEngine.Vector3.Lerp(previousPosition, position, (float)interpolationFactor);
                        transform.rotation = UnityEngine.Quaternion.Slerp(previousRotation, rotation, (float)interpolationFactor);
                    }
                    else
                    {
